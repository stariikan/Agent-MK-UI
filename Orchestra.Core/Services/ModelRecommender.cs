    ...
    public class ModelRecommender
    {
        public static readonly IReadOnlyList<ModelTier> Tiers = new List<ModelTier>
        {
            new()
            {
                ...
                ApproxDiskGb = 1.0,
                Description =
                    "Runs on almost anything, including CPU-only laptops. "
            ...
            new()
            {
                ...
                Description =
                    "Solid general coding assistant. Comfortable on 6-8GB GPUs "
            ...
            new()
            {
                ...
                Description =
                    "Noticeably stronger reasoning/code quality. Wants a 10GB+ GPU, "
            ...
            new()
            {
                ...
                Description =
                    "Best local quality this app recommends by default. Needs a "
        ...
        )
        {
            ...
            {
                notes.Add(
                    "Dedicated VRAM for non-NVIDIA GPUs is detected via Windows WMI, "
            ...
            {
                notes.Add(
                    "GPU VRAM could not be reliably detected (looked like the known "
            ...
            foreach (var tier in Tiers)
            {
                if (hasUsableGpu)
                {
                    if (effectiveVram!.Value >= tier.MinVramGb)
                        chosen = tier;
                }
                else
                {
                    if (effectiveRam >= tier.MinRamGbCpuOnly)
                        chosen = tier;
                }
            }
            ...
            {
                reason =
                    $"Selected based on {effectiveVram:F1}GB of GPU VRAM ({hardware.GpuName ?? "unknown GPU"}).";
            }
            else
            {
                reason =
                    $"No usable dedicated GPU detected; selected based on {effectiveRam:F1}GB "
                    + "of system RAM (CPU inference will be slower than GPU inference).";

                notes.Add(
                    "Running a coding LLM on CPU works but is significantly slower than "

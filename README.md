...
Primary use case: Local coding and software-engineering assistance
...
The runtime currently allows a configurable maximum number of tool steps, with a default of 12.
...
rather than using only a fixed percentage.
...
Agent-MK is intentionally split into several layers.
...
The Python runtime is designed to remain usable independently of the desktop application.

You can run the runtime directly in interactive mode:

python assistant.py

Or provide a task directly:

python assistant.py "Inspect the project and find the cause of the startup error."

The CLI supports selecting the workspace, provider, model, temperature, tool-step limit, dry-run mode, and command execution.
...
The runtime currently exposes an 8K generation ceiling through Ollama's num_predict setting.

The actual response can be shorter because the model can stop before reaching the limit.
...
The exact environment may change during development, but the current project is built around:
...
AI runtime
Python 3.10+
Ollama or another supported model endpoint
...
Install Ollama for Windows and download a supported model.

For example:

ollama pull qwen2.5-coder:14b
2. Clone the repository
git clone https://github.com/<your-username>/Agent-MK-UI.git
cd Agent-MK-UI
3. Open the solution
...
Use the project panel to inspect the project.
Describe the task in normal language.
Let the agent inspect only the files it needs.
Review the changes in the workspace.
Run validation/tests when appropriate.
...
Agent-MK is still under active development.
...
The Windows desktop application currently depends on the local Python runtime being correctly configured.
Roadmap

Possible future work includes:

improved repository/project indexing
stronger codebase navigation
better incremental file reading
smarter context summarization
more advanced validation and test execution
model-specific optimization profiles
richer change summaries and diffs
improved recovery from failed tool calls
better long-running task management
additional local model providers
deeper IDE/editor integration
Why Agent-MK?
